...
:is_backend_ready
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $h = Invoke-RestMethod -Uri '%BACKEND_URL%/api/healthz' -TimeoutSec 2; if ($h.status -ne 'ok') { exit 1 }; if ($h.live_signal_contract -ne '%REQUIRED_LIVE_SIGNAL_CONTRACT%') { exit 1 }; if ($h.trade_eval_contract -ne '%REQUIRED_TRADE_EVAL_CONTRACT%') { exit 1 }; if ($h.trade_excursion_contract -ne '%REQUIRED_TRADE_EXCURSION_CONTRACT%') { exit 1 }; if ($h.ai_strategy_catalog -ne '%REQUIRED_AI_STRATEGY_CATALOG%') { exit 1 }; if ($h.trade_summary_contract -ne '%REQUIRED_TRADE_SUMMARY_CONTRACT%') { exit 1 }; if ($h.minute_archive_contract -ne '%REQUIRED_MINUTE_ARCHIVE_CONTRACT%') { exit 1 }; $o = Invoke-RestMethod -Uri '%BACKEND_URL%/openapi.json' -TimeoutSec 2; $paths = $o.paths.PSObject.Properties.Name; if (-not ($paths -contains '%REQUIRED_BACKEND_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_AI_STRATEGY_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_STRATEGY_RUN_DETAIL_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_LIVE_SIGNAL_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_REVIEW_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_TRADE_REVIEW_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_TRADE_SUMMARY_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_TRADE_SUMMARY_GENERATION_ROUTE%')) { exit 1 }; $t = Invoke-RestMethod -Uri '%BACKEND_URL%/api/strategy-templates' -TimeoutSec 2; if (($t.items | ForEach-Object { $_.template_key }) -contains '%REQUIRED_STRATEGY_TEMPLATE%') { exit 0 }; exit 1 } catch { exit 1 }"
exit /b %ERRORLEVEL%
...
:is_frontend_ready
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Invoke-WebRequest -UseBasicParsing -Uri '%FRONTEND_URL%/' -TimeoutSec 2; if ($r.StatusCode -ne 200) { exit 1 }; $app = Invoke-WebRequest -UseBasicParsing -Uri '%FRONTEND_URL%/src/App.tsx' -TimeoutSec 2; if (-not $app.Content.Contains('%REQUIRED_FRONTEND_MARKER%') -or -not $app.Content.Contains('%REQUIRED_TRADE_SUMMARY_FRONTEND_MARKER%')) { exit 1 }; $h = Invoke-RestMethod -Uri '%FRONTEND_URL%/api/healthz' -TimeoutSec 2; if ($h.status -ne 'ok' -or $h.live_signal_contract -ne '%REQUIRED_LIVE_SIGNAL_CONTRACT%' -or $h.trade_eval_contract -ne '%REQUIRED_TRADE_EVAL_CONTRACT%' -or $h.trade_excursion_contract -ne '%REQUIRED_TRADE_EXCURSION_CONTRACT%' -or $h.ai_strategy_catalog -ne '%REQUIRED_AI_STRATEGY_CATALOG%' -or $h.trade_summary_contract -ne '%REQUIRED_TRADE_SUMMARY_CONTRACT%' -or $h.minute_archive_contract -ne '%REQUIRED_MINUTE_ARCHIVE_CONTRACT%') { exit 1 }; $o = Invoke-RestMethod -Uri '%FRONTEND_URL%/openapi.json' -TimeoutSec 2; $paths = $o.paths.PSObject.Properties.Name; if (-not ($paths -contains '%REQUIRED_BACKEND_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_AI_STRATEGY_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_STRATEGY_RUN_DETAIL_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_LIVE_SIGNAL_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_REVIEW_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_TRADE_REVIEW_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_TRADE_SUMMARY_ROUTE%')) { exit 1 }; if (-not ($paths -contains '%REQUIRED_TRADE_SUMMARY_GENERATION_ROUTE%')) { exit 1 }; exit 0 } catch { exit 1 }"
exit /b %ERRORLEVEL%
