...
Daily summary 的 PnL、胜率和盈亏比必须按已平仓 round-trip 计算：同一账号和标的下，仓位从 0 开始，B&S 或 S&B 回到 0 时结算一笔交易；多次开平仓必须拆成多笔。未平仓单边成交不进入胜率、盈亏比或已实现 PnL。
...
- KPI 分组：`trade_group_count` 是已平仓 round-trip 数；胜率和盈亏比不能用当日账号+标的净 PnL 代替。
