...
- 「数据下钻」tab 现在先展示全部、本月、本周或特定时间段筛选，再展示该时间范围内全部订单的统计指标和热力时间矩阵；矩阵按美股常规盘五大微观结构窗口和开仓 ATR Multiple 只读展示订单分布，并在摘要中标出最大盈利区和最大亏损区，底部按时间窗口汇总收益；没有缺 ATR 证据订单时不显示缺证据行，日期/标的下钻列表只受时间范围影响。
- 「下钻复盘」新增「数据下钻」和「盈亏复盘」两个模块 tab；「盈亏复盘」默认仅看已平仓亏损交易组，也可切到仅看盈利单。亏损视图可按全部、本月、本周或特定时间段筛选统计、原因分类汇总和订单明细，并用一级/二级原因饼图联动筛选列表；盈利视图只展示对应热力时间矩阵和订单明细，原因模块保持为空。热力时间矩阵按美股常规盘五大微观结构窗口和开仓 1min K 振幅 / 前 20 根 ATR 的倍数只读定位最大亏损区或最大盈利区，仍只写 Review Journal。
