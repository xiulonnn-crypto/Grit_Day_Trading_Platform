...
目标：在 Trade Replay 弹层内为已平仓亏损交易组提供精简原因下拉，记录本次亏损的主观归因；「成交记录」模块提供「仅看亏损单」勾选项但不提供单独「复盘」操作按钮；「下钻复盘」新增「亏损复盘」tab，集中查看全部亏损交易组的原因分类和明细入口。
...
- 「下钻复盘」默认展示「数据下钻」；切到「亏损复盘」后读取所有日期 closed 且 PnL 小于 0 的交易组，提供全部、本月、本周和特定时间段全局筛选，并用该时间范围控制统计指标、一级/二级原因分类汇总和亏损单列表。
- 亏损复盘统计指标整理在一行；一级原因和二级原因分别用饼图展示，并支持联动多选筛选订单明细；订单明细默认按时间倒序，也可按亏损金额倒序，每页 20 笔。
- 「亏损复盘」展示只读热力时间矩阵，按 09:30-10:30、10:30-11:30、11:30-13:30、13:30-15:00、15:00-16:00 五个美股常规盘微观结构窗口和开仓 ATR Multiple 统计亏损集中区，并只在摘要中展示最大亏损区；矩阵格不筛选订单明细。波动/冲击环境读取后端 `position_drawdown.entry_atr_multiple`，由本地 `market_minute_archives` 计算开仓 1min K 振幅 / 前 20 根 ATR；缺开仓前 20 根分钟线时显示缺 ATR 证据，不用已实现亏损或美元回撤回退，也不新增前端行情指标计算。
